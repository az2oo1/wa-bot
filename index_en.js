...
// 🔄 Auto-update table schema to add new media-limit columns
const colsToAdd = [
    'blocked_types TEXT',
    'blocked_action TEXT',
    'spam_types TEXT',
...
let currentQR = '';
let botStatus = 'Initializing system, please wait...';

const userTrackers = new Map();
const abortedMessages = new Set(); 
const spamMutedUsers = new Map(); 

// Media types metadata for UI generation
const mediaTypesMeta = [
    { id: 'text', icon: '📝', name: 'Text' },
    { id: 'image', icon: '🖼️', name: 'Images' },
    { id: 'video', icon: '🎥', name: 'Videos' },
    { id: 'audio', icon: '🎵', name: 'Audio' },
    { id: 'document', icon: '📄', name: 'Files' },
    { id: 'sticker', icon: '👾', name: 'Stickers' }
];
...
app.get('/', (req, res) => {
    ...
    <head>
        ...
        <title>Auto Moderator Dashboard</title>
    ...
    <body>
        ...
        <nav class="sidebar" id="sidebar">
            <div class="sidebar-logo">
                ...
                <div class="logo-text">Auto Moderator <small>Control Panel V6</small></div>
            </div>
            ...
            <div class="sidebar-footer">
                <button id="logoutBtn" onclick="logoutBot()" style="display:none; background: var(--red-dim); border-color: rgba(248,81,73,0.4); color: var(--red);">🚪 Disconnect</button>
                <button onclick="saveConfig()" style="background: var(--accent-dim); border-color: rgba(0,230,118,0.4); color: var(--accent); font-weight:700;">💾 Save</button>
            </div>
        </nav>
        ...
        <div class="main">
            <div class="topbar">
                <div style="display:flex;align-items:center;gap:12px;">
                    ...
                    <span class="topbar-title" id="topbarTitle">Connection Status</span>
                </div>
            ...
            <div class="page active" id="page-status">
                <div class="page-header">
                    ...
                    <p>Link your WhatsApp account by scanning the QR code or monitor the active connection</p>
                </div>
                <div class="card-grid">
                    <div class="card" style="grid-column: 1;">
                        ...
                        <div class="qr-wrap">
                            ...
                            <div id="qr-placeholder" style="text-align:center; color: var(--text-muted); padding: 20px 0;">
                                ...
                                <div style="font-size: 18px; font-weight: 700; color: var(--text);">Waiting for QR code...</div>
                                <div style="font-size: 14px; margin-top: 8px;">The code will appear here automatically</div>
                            </div>
                        </div>
                    </div>
                    <div style="display:flex; flex-direction:column; gap:20px;">
                        <div class="card success">
                            <div class="card-header"><h3 style="color:var(--accent);">📊 System Status</h3></div>
                            <div style="font-size:16px; color:var(--text-muted); line-height:2.2;">
                                <div>🤖 <strong style="color:var(--text);">Bot:</strong> <span id="status-text-detail">${botStatus}</span></div>
                                <div>🗄️ <strong style="color:var(--text);">Database:</strong> <span style="color:var(--accent);">Connected ✓</span></div>
                                <div>🌐 <strong style="color:var(--text);">Port:</strong> <span style="color:var(--accent);">3000 ✓</span></div>
                            </div>
                        </div>
                        <div class="card">
                            <div class="card-header"><h3>🔗 Default Admin Group</h3></div>
                            <div class="field-group">
                                <label class="field-label">Group ID (for receiving alerts)</label>
                                <input type="text" id="defaultAdminGroup" value="${config.defaultAdminGroup}" dir="ltr" style="font-family: monospace; font-size:13px;">
                            </div>
                        </div>
                        <div class="card info">
                            <div class="card-header"><h3 style="color:var(--blue);">ℹ️ Getting Started</h3></div>
                            <div style="font-size:14px; color:var(--text-muted); line-height:2.2;">
                                <div>1️⃣ Scan the QR code with your WhatsApp</div>
                                <div>2️⃣ Add the bot as an admin in your groups</div>
                                <div>3️⃣ Open Settings and customize your rules</div>
                                <div>4️⃣ Click <strong style="color:var(--accent);">💾 Save</strong> to apply changes</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="page" id="page-blacklist">
                <div class="page-header">
                    ...
                    <p>Blacklisted numbers are instantly kicked from any group where the bot is an admin</p>
                </div>
                <div class="card-grid">
                    <div class="card danger">
                        <div class="card-header">
                            <h3 style="color:var(--red);">➕ Add Number to Blacklist</h3>
                            <span style="font-size: 13px; color: var(--text-muted); background:var(--red-dim); padding:4px 10px; border-radius:20px;">Saved instantly to DB</span>
                        </div>
                        <div class="field-group">
                            <label class="field-label">Phone Number (without +)</label>
                            <div class="input-with-btn">
                                <input type="text" id="newBlacklistNumber" placeholder="e.g. 966582014941" onkeypress="if(event.key==='Enter'){event.preventDefault();addBlacklistNumber();}">
                                <button type="button" class="btn btn-danger" onclick="addBlacklistNumber()">+ Ban</button>
                            </div>
                        </div>
                        <label class="field-label">Currently Banned Numbers</label>
                            ...
                            <div class="toggle-row danger" style="margin-bottom:0;">
                                <div class="toggle-left">
                                    ...
                                    <div class="toggle-label danger">
                                        ...
                                        <small>Instant kick on join attempt or message</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="card warning">
                            <div class="card-header"><h3 style="color:var(--orange);">🧹 Retroactive Mass Purge</h3></div>
                            <p style="font-size:14px; color:var(--text-muted); margin-bottom: 18px; line-height:1.8;">The bot will scan all groups where it is an admin and immediately kick anyone on the blacklist.</p>
                            <button type="button" id="purgeBtn" class="btn btn-warning btn-full" onclick="purgeBlacklisted()">
                                🧹 Execute Full Purge Now
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div class="page" id="page-general">
                <div class="page-header">
                    ...
                    <p>Applied to all groups that do not have custom settings</p>
                </div>
                <div class="card-grid">
                    <div class="card">
                        <div class="card-header"><h3>🔤 Banned Word Filter</h3></div>
                        <div class="toggle-row" style="margin-bottom:18px;">
                            <div class="toggle-left">
                                ...
                                <div class="toggle-label">Enable Word Filter<small>Instantly delete any message containing a banned word</small></div>
                            </div>
                        </div>
                        <div class="field-group">
                            <label class="field-label">Default Banned Words</label>
                            <div class="input-with-btn">
                                <input type="text" id="newDefaultWord" placeholder="Enter banned word..." onkeypress="if(event.key==='Enter'){event.preventDefault();addDefaultWord();}">
                                <button type="button" class="btn btn-primary" onclick="addDefaultWord()">+ Add</button>
                            </div>
                            <div id="defaultWordsContainer" class="chip-container"></div>
                        </div>
                    </div>
                    <div class="card">
                        <div class="card-header"><h3>🚦 Auto Action</h3></div>
                        <div class="toggle-row pink">
                            <div class="toggle-left">
                                ...
                                <div class="toggle-label pink">
                                    ...
                                    <small>Skip admin vote when a violation is detected</small>
                                </div>
                            </div>
                        </div>
                        <div style="margin-top:20px; padding:16px; background:var(--input-bg); border-radius:10px; border:1px solid var(--card-border);">
                            <div style="font-size:13px; color:var(--text-muted); line-height:2;">
                                <div>🔴 <strong style="color:var(--text);">Enabled:</strong> Instant delete + auto kick</div>
                                <div>🟡 <strong style="color:var(--text);">Disabled:</strong> Delete + admin vote poll</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="page" id="page-spam">
                <div class="page-header">
                    <h2>🛡️ Anti-Spam</h2>
                    <p>Detect repeated messages within a 15-second window</p>
                </div>
                    ...
                    <div class="toggle-row warning" style="margin-bottom:0; border-radius:10px;">
                        <div class="toggle-left">
                            ...
                            <div class="toggle-label warning">
                                ...
                                <small>Monitor each user's send rate within a 15-second window</small>
                            </div>
                        </div>
                    </div>
                        ...
                        <div style="border-top: 1px dashed rgba(255,171,64,0.3); padding-top: 20px;">
                            ...
                            <div class="field-row" style="margin-bottom:20px;">
                                <div class="field-group" style="margin-bottom:0;">
                                    <label class="field-label">Action on Detection</label>
                                    <select id="spamAction">
                                        <option value="poll" ${config.spamAction === 'poll' ? 'selected' : ''}>🗳️ Admin Vote Poll</option>
                                        <option value="auto" ${config.spamAction === 'auto' ? 'selected' : ''}>🔨 Auto Kick & Ban</option>
                                    </select>
                                </div>
                                <div class="field-group" style="margin-bottom:0;">
                                    <label class="field-label">Duplicate Text Limit</label>
                                    <input type="number" id="spamDuplicateLimit" value="${config.spamDuplicateLimit}" min="2" max="15" placeholder="3">
                                </div>
                            </div>

                            <!-- Per-type limits -->
                            <label class="field-label" style="margin-bottom:12px;">⏱️ Per-Type Limits within 15 Seconds</label>
                            <p style="font-size:13px; color:var(--text-muted); margin-bottom:14px;">Enable ✓ the type to monitor, then set the maximum messages allowed</p>
            ...
            <div class="page" id="page-media">
                <div class="page-header">
                    <h2>🛑 Media Filter</h2>
                    <p>Absolute block for specific types — deletion happens instantly regardless of any other setting</p>
                </div>
                <div class="card-grid">
                    <div class="card danger">
                        <div class="card-header"><h3 style="color:var(--red);">📁 Select Blocked Types</h3></div>
                        <p style="font-size:14px; color:var(--text-muted); margin-bottom:18px;">Any message of these types will be automatically deleted without exception.</p>
                    ...
                    <div class="card danger">
                        <div class="card-header"><h3 style="color:var(--red);">⚡ Action on Detection</h3></div>
                        <div class="field-group">
                            <label class="field-label">What should the bot do when a blocked type is sent?</label>
                            <select id="globalBlockedAction" style="font-size:15px; padding:14px;">
                                <option value="delete" ${config.blockedAction === 'delete' ? 'selected' : ''}>🗑️ Delete message only (silently)</option>
                                <option value="poll" ${config.blockedAction === 'poll' ? 'selected' : ''}>🗳️ Delete + open admin vote poll</option>
                                <option value="auto" ${config.blockedAction === 'auto' ? 'selected' : ''}>🔨 Delete + auto kick & ban</option>
                            </select>
                        </div>
                        <div style="margin-top:16px; padding:16px; background:var(--red-dim); border-radius:10px; border:1px solid rgba(255,82,82,0.2);">
                            <div style="font-size:13px; color:var(--text-muted); line-height:2.2;">
                                <div>🗑️ <strong style="color:var(--text);">Delete only:</strong> Silent, sender is unaware</div>
                                <div>🗳️ <strong style="color:var(--text);">Vote:</strong> Notify admins to make a decision</div>
                                <div>🔨 <strong style="color:var(--text);">Auto kick:</strong> Strongest action, instant ban</div>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="page" id="page-ai">
                <div class="page-header">
                    <h2>🧠 AI Moderator</h2>
                    <p>Content analysis using a local Ollama LLM model</p>
                </div>
                <div class="card-grid">
                    <div class="card info">
                        <div class="card-header">
                            <h3 style="color:var(--blue);">🔌 Enable AI</h3>
                            <button type="button" class="btn btn-blue btn-sm" onclick="openOllamaModal()">⚙️ Server Setup</button>
                        </div>
                        <div class="toggle-row blue" style="margin-bottom:12px;">
                            <div class="toggle-left">
                                ...
                                <div class="toggle-label blue">Analyze Text with AI<small>Inspect every text message before allowing it</small></div>
                            </div>
                        </div>
                        <div class="toggle-row purple">
                            <div class="toggle-left">
                                ...
                                <div class="toggle-label purple">Analyze Images (Vision AI)<small>Requires a Vision-capable model such as llava</small></div>
                            </div>
                        </div>
                    </div>
                    <div class="card" id="aiPromptContainer">
                        <div class="card-header"><h3>📝 AI Instructions</h3></div>
                        <div class="field-group">
                            <label class="field-label">Describe the prohibited content to the model</label>
                        ...
                        <div style="font-size:12px; color:var(--text-muted); padding:10px; background:var(--input-bg); border-radius:8px; border:1px solid var(--card-border);">
                            💡 Example: "Block any message containing commercial ads, suspicious links, or offensive content"
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="page" id="page-groups">
                <div class="page-header" style="display:flex; justify-content:space-between; align-items:flex-start;">
                    <div>
                        <h2>👥 Custom Groups</h2>
                        <p>Per-group settings — override the global defaults</p>
                    </div>
                    <button type="button" class="btn btn-blue" onclick="addGroup()">+ Add Group</button>
                </div>
                <div id="groupsContainer"></div>
            </div>

            <!-- Toast -->
            <div id="saveMsgToast" class="toast">✅ Saved to database successfully!</div>
        ...
        <div id="ollamaModal" class="modal">
            <div class="modal-content">
                <div class="modal-header">
                    <h3 style="color:var(--blue);">🔗 Ollama Server Settings</h3>
                    <button class="close-modal" onclick="closeOllamaModal()">×</button>
                </div>
                <div class="field-group">
                    <label class="field-label">Server URL (Endpoint)</label>
                    <input type="text" id="ollamaUrl" value="${config.ollamaUrl}" dir="ltr" style="font-family:monospace;">
                </div>
                <div class="field-group">
                    <label class="field-label">Model Name</label>
                    <input type="text" id="ollamaModel" value="${config.ollamaModel}" dir="ltr" style="font-family:monospace;" placeholder="e.g. llava">
                    <div style="font-size:12px; color:var(--text-muted); margin-top:6px;">For images use a Vision model such as <code>llava</code></div>
                </div>
                <button type="button" class="btn btn-primary btn-full" onclick="closeOllamaModal()">Save & Close</button>
            </div>
        </div>
        ...
        <div id="debuggerModal" class="modal">
            <div class="modal-content" style="max-width:800px; background:#0d1117; border-color:#21262d;">
                <div class="modal-header">
                    <h3 style="color:var(--accent); font-family:monospace;">⬛ Live Event Log</h3>
                    <button class="close-modal" onclick="closeDebuggerModal()">×</button>
                </div>
                <div id="terminalOutput"></div>
                <button type="button" class="btn btn-ghost btn-full" style="margin-top:14px;" onclick="closeDebuggerModal()">Close</button>
            </div>
        </div>
        ...
        <script>
                ...
                try {
                    ...
                    let html = logs.map(l => {
                        let styled = l.replace(/\\[ERROR\\]/g, '<span style="color:#ff3b30">[ERROR]</span>')
                                      ...
                                      .replace(/\\[CHECK\\]/g, '<span style="color:#ffeb3b">[CHECK]</span>')
                                      .replace(/\\[SECURITY\\]/g, '<span style="color:#ff9800">[SECURITY]</span>')
                                      .replace(/\\[CLEANUP\\]/g, '<span style="color:#9c27b0">[CLEANUP]</span>');
            ...
            async function logoutBot() {
                if(confirm('Are you sure you want to log out from the WhatsApp account? The bot will be disconnected.')) {
                    document.getElementById('status-text').innerText = 'Logging out...';
            ...
            // Navigation
            const pageTitles = {
                'page-status': 'Connection Status',
                'page-blacklist': 'Blacklist',
                'page-general': 'General Settings',
                'page-spam': 'Anti-Spam',
                'page-media': 'Media Filter',
                'page-ai': 'AI Moderator',
                'page-groups': 'Custom Groups'
            };
            ...
            let groupsArr = Object.keys(groupsConfigObj).map(key => ({
                ...
                welcomeMessageText: groupsConfigObj[key].welcomeMessageText || 'Welcome {user} to our group!',
            ...
            async function purgeBlacklisted() {
                if(!confirm('⚠️ Warning: This will make the bot scan all groups and immediately kick anyone on the blacklist. Are you sure?')) return;
                const btn = document.getElementById('purgeBtn');
                const originalText = btn.innerText;
                btn.innerText = '⏳ Scanning groups and kicking members...';
                btn.disabled = true;
                try {
            ...
            function renderGroups() {
                ...
                groupsArr.forEach((group, groupIndex) => {
                    ...
                    // Generate blocked types checkboxes
                    ...
                    <div class="group-card">
                        <div class="group-card-header">
                            <div class="group-card-title">
                                <span>👥</span>
                                Group \${groupIndex + 1}
                                \${group.id ? \`<span class="group-id-badge">\${group.id.split('@')[0].slice(-8)}...</span>\` : '<span style="color:var(--orange);font-size:12px;">No ID set</span>'}
                            </div>
                            <button type="button" class="btn btn-danger btn-sm" onclick="removeGroup(\${groupIndex})">Remove</button>
                        </div>
                        <div class="group-card-body">

                            <div class="field-group">
                                <label class="field-label">Target Group ID</label>
                                <input type="text" placeholder="e.g. 120363000000000000@g.us" dir="ltr" style="font-family:monospace;" value="\${group.id}" onchange="updateGroupData(\${groupIndex}, 'id', this.value)">
                            </div>
                            <div class="field-group">
                                <label class="field-label">Custom Admin Group (leave empty to use default)</label>
                                <input type="text" dir="ltr" style="font-family:monospace;" value="\${group.adminGroup}" onchange="updateGroupData(\${groupIndex}, 'adminGroup', this.value)">
                            </div>

                            <!-- Blocked types -->
                            <div class="sub-panel red" style="margin-bottom:12px;">
                                <h4 style="color:var(--red);">🛑 Absolutely Blocked Types</h4>
                                <div class="cb-group" style="margin-bottom:10px;">\${blockedChecks}</div>
                                <label class="field-label">Block Action</label>
                                <select onchange="updateGroupData(\${groupIndex}, 'blockedAction', this.value)">
                                    <option value="delete" \${group.blockedAction === 'delete' ? 'selected' : ''}>Delete message only</option>
                                    <option value="poll" \${group.blockedAction === 'poll' ? 'selected' : ''}>Delete + admin vote poll</option>
                                    <option value="auto" \${group.blockedAction === 'auto' ? 'selected' : ''}>Delete + auto kick</option>
                                </select>
                            </div>
                            ...
                            <div class="toggle-row warning" style="margin-bottom:0; border-radius:\${group.enableAntiSpam ? '10px 10px 0 0' : '10px'};">
                                <div class="toggle-left">
                                    ...
                                    <div class="toggle-label warning">Anti-Spam<small>Detect repeated messages within 15 seconds</small></div>
                                </div>
                            </div>
                            ...
                            ">
                                <div class="sub-panel orange" style="border-top:none; border-radius:0 0 10px 10px;">
                                    <h4 style="color:var(--orange);">⏱️ Per-Type Limits (15 seconds)</h4>
                                    ...
                                    <div class="field-row" style="border-top:1px dashed rgba(255,171,64,0.3);padding-top:12px;margin-top:4px;">
                                        ...
                                        <div>
                                            ...
                                            <select onchange="updateGroupData(\${groupIndex}, 'spamAction', this.value)">
                                                <option value="poll" \${group.spamAction === 'poll' ? 'selected' : ''}>🗳️ Admin Vote Poll</option>
                                                <option value="auto" \${group.spamAction === 'auto' ? 'selected' : ''}>🔨 Auto Kick</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="toggle-row green" style="margin-bottom:0; border-radius:\${group.enableWelcomeMessage ? '10px 10px 0 0' : '10px'};">
                                <div class="toggle-left">
                                    ...
                                    <div class="toggle-label green">Welcome Message on Join<small>Sent by the bot to every new member</small></div>
                                </div>
                            </div>
                            ...
                            ">
                                <div class="sub-panel" style="border-top:none; border-radius:0 0 10px 10px; border-color:rgba(100,200,120,0.3);">
                                    <label class="field-label">Message text ({user} for mention)</label>
                            ...
                            <div class="toggle-row danger" style="margin-bottom:12px;">
                                <div class="toggle-left">
                                    ...
                                    <div class="toggle-label danger">Enable Blacklist<small>Instantly kick any banned number</small></div>
                                </div>
                            </div>

                            <!-- Word filter toggle + slide panel -->
                            <div class="toggle-row warning" style="margin-bottom:0; border-radius:\${group.enableWordFilter ? '10px 10px 0 0' : '10px'};">
                                <div class="toggle-left">
                                    ...
                                    <div class="toggle-label warning">Banned Word Filter<small>Instantly delete messages containing banned words</small></div>
                                </div>
                            </div>
                                ...
                                <div class="sub-panel orange" style="border-top:none; border-radius:0 0 10px 10px;">
                                    ...
                                    <div class="toggle-row" style="margin-bottom:14px; background:rgba(255,255,255,0.04); border-color:rgba(255,171,64,0.25);">
                                        <div class="toggle-left">
                                            ...
                                            <div class="toggle-label">Apply global word list too<small>Add the global banned words to this group</small></div>
                                        </div>
                                    </div>
                                    <label class="field-label">Custom banned words for this group</label>
                                    <div class="input-with-btn" style="margin-bottom:10px;">
                                        <input type="text" id="newGroupWord_\${groupIndex}" placeholder="Enter word..." onkeypress="if(event.key==='Enter'){event.preventDefault();addGroupWord(\${groupIndex});}">
                                        <button type="button" class="btn btn-primary btn-sm" onclick="addGroupWord(\${groupIndex})">+ Add</button>
                                    </div>
                            ...
                            <div class="toggle-row blue" style="margin-bottom:12px;">
                                <div class="toggle-left">
                                    ...
                                    <div class="toggle-label blue">AI Moderator (Text)</div>
                                </div>
                            </div>
                            <div class="toggle-row purple" style="margin-bottom:12px;">
                                <div class="toggle-left">
                                    ...
                                    <div class="toggle-label purple">Image Analysis (Vision)</div>
                                </div>
                            </div>
                            <div class="toggle-row pink">
                                <div class="toggle-left">
                                    ...
                                    <div class="toggle-label pink">Direct Action (Skip Vote)</div>
                                </div>
                            </div>
            ...
            function addGroup() {
                groupsArr.push({ 
                    ...
                    enableWelcomeMessage: false, welcomeMessageText: 'Welcome {user} to our group!',
                    blockedTypes: [], blockedAction: 'delete', 
                    spamTypes: ['text', 'image', 'video', 'audio', 'document', 'sticker'],
                    spamLimits: {text:7, image:3, video:2, audio:3, document:3, sticker:3}
                });
            ...
            function removeGroup(index) {
                if(confirm('Are you sure you want to delete the custom settings for this group?')) {
            ...
            function toggleGroupPanel(groupIndex, type, enabled) {
                ...
                const panel = document.getElementById(\`group_\${type}_panel_\${groupIndex}\`);
            ...
            setInterval(async () => {
                try {
                    ...
                    if(data.status.includes('Connected and ready')) {
                        dot.className = 'status-dot online';
                        document.getElementById('logoutBtn').style.display = 'block';
                    } else if(data.status.includes('QR') || data.status.includes('Waiting')) {
            ...
            async function saveConfig() {
                ...
                if(res.ok) showToast('✅ Saved to database successfully!');
                else showToast('❌ Save failed, check the server');
            }
...
app.post('/api/blacklist/add', (req, res) => {
    if(req.body.number) {
        try {
            ...
            console.log(`[SECURITY] Number added to blacklist via panel: ${req.body.number}`);
        } catch(e) {}
    }
    res.sendStatus(200);
});
...
app.post('/api/blacklist/remove', (req, res) => {
    if(req.body.number) {
        try {
            ...
            console.log(`[SECURITY] Number removed from blacklist via panel: ${req.body.number}`);
        } catch(e) {}
    }
    res.sendStatus(200);
});
...
app.post('/api/blacklist/purge', async (req, res) => {
    if (!client.info || !client.info.wid) {
        return res.status(400).json({ error: 'Bot is not connected. Please wait.' });
    }

    try {
        console.log(`[CLEANUP] Starting full group purge...`);
        const blacklistRows = db.prepare('SELECT number FROM blacklist').all();
        const blacklistArr = blacklistRows.map(r => r.number);

        if (blacklistArr.length === 0) return res.json({ message: 'The blacklist is empty.' });
                ...
                if (botIsAdmin) {
                    ...
                    if (usersToKick.length > 0) {
                        try {
                            ...
                            console.log(`[SECURITY] 🧹 Kicked ${usersToKick.length} banned users from: ${chat.name}`);
        ...
        console.log(`[CLEANUP] Purge complete. Kicked ${kickedCount} users.`);
        res.json({ message: `Purge completed successfully! Kicked (${kickedCount}) banned members.` });
    } catch (error) {
        res.status(500).json({ error: 'A server error occurred during the purge.' });
    }
});
...
app.post('/save', (req, res) => {
    try {
        ...
        console.log('[CHECK] 💾 Settings saved successfully.');
        res.sendStatus(200);
    } catch(e) {
        console.error('[ERROR] Failed to save to database:', e.message);
        res.sendStatus(500);
    }
});
...
client.on('ready', async () => {
    ...
    currentQR = '';
    console.log('WhatsApp account linked successfully!');
    
    try {
        const chats = await client.getChats();
        let groupsList = '--- Groups List ---\n\n';
        chats.filter(c => c.isGroup).forEach(c => {
            groupsList += `Name: ${c.name}\nID: ${c.id._serialized}\n-----------------------\n`;
        });
...
client.on('authenticated', () => {
    botStatus = 'Logged in successfully, fetching data...';
    currentQR = '';
});

client.on('disconnected', async (reason) => {
    botStatus = 'Logged out from account...';
    currentQR = '';
    ...
    try {
        ...
        for (const participantId of notification.recipientIds) {
            ...
            if (isBlacklistEnabledForGroup) {
                ...
                if (isBanned) {
                    console.log(`[SECURITY] 🛡️ Banned number attempted to join (${cleanJoinedId}). Kicking...`);
                    isKicked = true;
                    ...
                    setTimeout(async () => {
                        try {
                            ...
                            const reportText = `🛡️ *Protection (Blacklist)*\nA banned number attempted to join "${chat.name}" and was kicked.\nNumber: @${cleanJoinedId.split('@')[0]}`;
    ...
    try {
        ...
        if (chat.isGroup) {
            ...
            if (isBlacklistEnabled) {
                ...
                if (isBanned) {
                    console.log(`[SECURITY] 🛡️ Banned number sent a message. Kicking...`);
            ...
            if (blockedTypes.includes(internalMsgType)) {
                console.log(`[SECURITY] 🛑 Absolutely blocked type detected (${internalMsgType}). Deleting.`);
                ...
                if (blockedAction === 'auto') {
                    try {
                        ...
                        const reportText = `🚨 *Auto Ban (Blocked Type)*\nMember sent a (${internalMsgType}) in "${chat.name}" and was kicked.\n👤 *Sender:* @${cleanAuthorId.split('@')[0]}`;
                        await client.sendMessage(targetAdminGroup, reportText, { mentions: [cleanAuthorId] });
                    } catch(e){}
                } else if (blockedAction === 'poll') {
                    const pollTitle = `🚨 Violation alert in "${chat.name}"\nSender: @${cleanAuthorId.split('@')[0]}\nReason: Sent a blocked type (${internalMsgType})\n\nKick the number${isBlacklistEnabled ? ' and add to blacklist' : ''}?`;
                    const poll = new Poll(pollTitle, isBlacklistEnabled ? ['Yes, kick & ban', 'No'] : ['Yes, kick', 'No']);
            ...
            if (isAntiSpamEnabled) {
                ...
                if (spamTypes.includes(internalMsgType)) {
                    ...
                    if (typeCount >= typeLimit) {
                        isSpamFlagged = true;
                        spamFlagReason = `Sending (${internalMsgType}) faster than allowed limit (${typeLimit} in 15s)`;
                    }
                }
                    ...
                    for (const m of tracker) {
                        if (m.type === 'text' && m.text && m.text.trim().length > 0) {
                            ...
                            if (textCounts[m.text] >= spamDuplicateLimit) {
                                isSpamFlagged = true;
                                spamFlagReason = `Same text repeated ${textCounts[m.text]} times`;
                                break;
                            }
                        }
                    }
                ...
                if (isSpamFlagged) {
                    console.log(`[SECURITY] 🚨 Spam detected in (${chat.name}): ${spamFlagReason}`);
                    ...
                    if (spamAction === 'auto') {
                        try {
                            ...
                            const reportText = `🚨 *Auto Ban (Spam)*\nMember was kicked from "${chat.name}"${isBlacklistEnabled ? ' and added to blacklist' : ''}.\n\n👤 *Sender:* @${senderId.split('@')[0]}\n📋 *Reason:* ${spamFlagReason}`;
                            await client.sendMessage(targetAdminGroup, reportText, { mentions: [senderId] });
                        } catch(e) {}
                    } else {
                        const pollOptions = isBlacklistEnabled ? ['Yes, kick & ban', 'No, just delete'] : ['Yes, kick', 'No'];
                        const pollTitle = `🚨 Spam alert in "${chat.name}"\nSender: @${senderId.split('@')[0]}\nReason: ${spamFlagReason}\n\nKick${isBlacklistEnabled ? ' and blacklist' : ''} the number?`;
            ...
            console.log(`[CHECK] Processing message in (${chat.name}) | words(${isWordFilterEnabled}), AI(${isAIFilterEnabled})`);
            ...
            if (isWordFilterEnabled && forbiddenWords.length > 0 && msg.body) {
                ...
                if (matchedWord) {
                    isViolating = true;
                    violationReason = `Exact match with banned word: [${matchedWord}]`;
                }
            }
            ...
            if (canSendToAI) {
                try {
                    const msgText = msg.body || '[Image with no caption]';
                    const aiPromptText = `You are a strict group moderator. Your instructions are: ${config.aiPrompt}\n\nBased on the instructions, is this content a violation? Answer with "yes" or "no" only.\nContent: "${msgText}"`;
                    ...
                    if (data.response && data.response.toLowerCase().includes('yes')) {
                        isViolating = true;
                        violationReason = 'Classified as a violation by AI';
                    }
                } catch (error) {}
            }
            ...
            if (isViolating) {
                ...
                const messageContent = msg.body || '[Media attachment]';
                await msg.delete(true); 
                ...
                if (isAutoActionEnabled) {
                    try {
                        ...
                        const reportText = `🚨 *Auto Action & Ban Report*\nViolating content was deleted and member was kicked from "${chat.name}".\n\n👤 *Sender:* @${senderId.split('@')[0]}\n📋 *Reason:* ${violationReason}\n📝 *Deleted text:*\n"${messageContent}"`;
                        await client.sendMessage(targetAdminGroup, reportText, { mentions: [senderId] });
                    } catch(e) {}
                } else {
                    const pollOptions = isBlacklistEnabled ? ['Yes, kick & ban', 'No, just delete'] : ['Yes, kick', 'No'];
...
client.on('vote_update', async vote => {
    ...
    if (pendingBans.has(pollId)) {
        if (vote.selectedOptions && vote.selectedOptions.length > 0) {
            ...
            if (selectedOption.toLowerCase().includes('yes')) {
                ...
                const replyText = data.isBlacklistEnabled ? '✅ *Kick applied and number added to blacklist successfully.*' : '✅ *Kick applied successfully.*';
                await data.pollMsg.reply(replyText);

            } else if (selectedOption.toLowerCase().includes('no')) {
                await data.pollMsg.reply('🛑 *Kick cancelled based on admin vote.*');
            }
